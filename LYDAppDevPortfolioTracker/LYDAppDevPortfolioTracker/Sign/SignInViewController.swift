...
class SignInViewController: UIViewController {
    ...
    @IBAction func guestSign(_ sender: Any) {
    ...
    func sign(_ signIn: GIDSignIn!, didSignInFor user: GIDGoogleUser!, withError error: Error!) {
        ...
        Auth.auth().signIn(with: credntial) { (authResult, error) in
            ...
            DispatchQueue.main.async {
                ...
                userDefault.set(uuid, forKey: "token")
                let signSB = UIStoryboard(name: "Main", bundle: nil)
                let mainsNavigationController = signSB.instantiateViewController(identifier: "PersonInfoViewController")
